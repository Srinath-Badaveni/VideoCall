...
const Login = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        setError("");
        try {
            const res    = await fetch(`${server_api}/api/v1/users/login`, {
                method: "POST",
    ...
    return (
        <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 'clamp(.75rem, 4vw, 2rem)', position: 'relative', overflow: 'hidden' }}>
            {/* Ambient blobs */}
            <div style={{ position: 'fixed', top: '-20%', right: '-10%', width: '60vw', height: '60vw', borderRadius: '50%', background: 'radial-gradient(circle,rgba(99,102,241,.2) 0%,transparent 70%)', pointerEvents: 'none' }} />
            <div style={{ position: 'fixed', bottom: '-20%', left: '-10%', width: '55vw', height: '55vw', borderRadius: '50%', background: 'radial-gradient(circle,rgba(139,92,246,.15) 0%,transparent 70%)', pointerEvents: 'none' }} />
            ...
            <div className="glass anim-fade-up" style={{ width: '100%', maxWidth: 460, padding: 'clamp(1.25rem, 6vw, 2.5rem)', position: 'relative', zIndex: 1 }}>
                ...
                <h1 style={{ fontSize: '1.75rem', fontWeight: 800, letterSpacing: '-0.75px', marginBottom: '.5rem' }}>Welcome back</h1>
                <p style={{ color: 'var(--text-secondary)', fontSize: '.875rem', marginBottom: '2rem' }}>Sign in to your account to continue</p>
                ...
                <p style={{ textAlign: 'center', fontSize: '.875rem', color: 'var(--text-secondary)' }}>
                    ...
                    <button onClick={() => navigate("/signup")} style={{ background: 'none', border: 'none', color: 'var(--accent-hover)', fontWeight: 700, cursor: 'pointer', fontSize: '.875rem' }}>
                        Sign up free
                    </button>
                </p>
            </div>
